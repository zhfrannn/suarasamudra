document.addEventListener('DOMContentLoaded', function() {
    ...
    // Mengambil data cerita dari file JSON, lalu memanggil fungsi initializeMap
    fetch('assets/data/stories.json')
        .then(response => response.json())
        .then(data => {
            initializeMap(data); // Panggil fungsi peta setelah data siap
        })
        .catch(error => console.error('Error memuat data cerita:', error));
    ...
    function showQuiz() {
        if (!quizContainer) return; // Pengaman jika elemen tidak ditemukan
